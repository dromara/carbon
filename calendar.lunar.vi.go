...
var (
	PI               = math.Pi
	vLunarHeavenStem = []string{"Giáp", "Ất", "Bính", "Đinh", "Mậu", "Kỷ", "Canh", "Tân", "Nhâm", "Quý"}
...
// reference document and algorithum
...
func dateToJuliusDay(dd, mm, yy int) int {
	...
	juliusDay := integerToFloat(dd) + mathFloor((153*m+2)/5) + 365*y + mathFloor(y/4) - math.Floor(y/100) + mathFloor(y/400) - 32045
	if juliusDay < 2299161 {
		// julius calendar
		juliusDay = integerToFloat(dd) + mathFloor((153*m+2)/5) + 365*y + mathFloor(y/4) - 32083
	}
...
func solarToLunar(dd, mm, yy, tz int) (lunarD, lunarM, lunarY, lunarLeap, lunarMonthLeap, juliusDayNumber int) {
	...
	k := floor((integerToFloat(juliusDayNumber) - 2415021.076998695) / 29.530588853)
	...
	lunarM = diff + 11
	if b11-a11 > 365 {
		leapMonthOffset := getLeapMonthOffset(integerToFloat(a11), tz)
...
func (c Carbon) VLunar() (l vlunar) {
	l.isInvalid, l.isLeapYear, l.leapMonth = false, 0, 0
	if c.IsInvalid() {
		l.Error = c.Error
		l.isInvalid = true
		return
	...
	l.day, l.month, l.year, l.isLeapYear, l.leapMonth, l.juliusDay = solarToLunar(c.Day(), c.Month(), c.Year(), l.timeZone)
...
func (l vlunar) YearHeavenStem() string {
	if l.isInvalid {
		return ""
	}
	return vLunarHeavenStem[(l.year+6)%10]
}
...
func (l vlunar) Animal() string {
	...
	return vLunarTimes[(l.year+8)%MonthsPerYear]
}

func (l vlunar) YearName() string {
	if l.isInvalid {
		return ""
	}
	return fmt.Sprintf("%s %s", l.YearHeavenStem(), l.Animal())
}
...
func (l vlunar) LuckyHour() string {
	...
	chiOfDay := (l.juliusDay + 1) % 12
	luckyHour := vLunarLuckyHours[chiOfDay%6]

	return luckyHour
}

func (l vlunar) LuckyHours() []LuckyHour {
	ret := []LuckyHour{}
...
func (l vlunar) MonthHeavenStem() string {
	if l.isInvalid {
		return ""
	}
	return vLunarHeavenStem[((l.year*12)+l.month+3)%10]
}

func (l vlunar) MonthAnimal() string {
	if l.isInvalid {
		return ""
	}
	return vLunarTimes[(l.month+1)%12]
}

func (l vlunar) MonthName() string {
	if l.isInvalid {
		return ""
	}

	return fmt.Sprintf("%s %s", l.MonthHeavenStem(), l.MonthAnimal())
}
...
func (l vlunar) DayHeavenStem() string {
	if l.isInvalid {
		return ""
	}
	return vLunarHeavenStem[(l.juliusDay+9)%10]
}

func (l vlunar) DayAnimal() string {
	if l.isInvalid {
		return ""
	}
	return vLunarTimes[(l.juliusDay+1)%12]
}

func (l vlunar) DayName() string {
	if l.isInvalid {
		return ""
	}
	return fmt.Sprintf("%s %s", l.DayHeavenStem(), l.DayAnimal())
}

// ToYearString outputs a string in lunar year format like "2020".
...
// ToMonthString outputs a string in lunar month format like "1".
...
// ToDayString outputs a string in lunar day format like "1".
