...
func TestCarbon_Issue240(t *testing.T) {
	...
	assert.True(t, person.Birthday1.IsValid())
	assert.True(t, person.Birthday2.IsValid())
}
...
func TestCarbon_Issue243(t *testing.T) {
	...
	assert.Equal(t, "PRC", project.StartDate.Timezone())
	assert.Equal(t, "PRC", project.EndDate.Timezone())
}
