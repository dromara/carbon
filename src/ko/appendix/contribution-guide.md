...
### 1단계: 언어 템플릿 파일 복사
 ...
 ```
여기서 `xx`는 언어 코드(`zh`, `en`, `ja` 등), `YY`는 국가 또는 지역 코드(`CN`, `US`, `CA` 등)입니다. 언어 코드와 국가 코드가 같으면 국가 코드를 생략할 수 있습니다(예: `ru-RU`는 `ru`로 생략 가능). 전체 언어 코드 및 국가 코드는 [언어 코드](language-codes.md)를 참조하세요.
