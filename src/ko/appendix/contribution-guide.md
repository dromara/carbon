...
### 1단계: 언어 템플릿 파일 복사
 ...
 ```
여기서 `xx`는 추가할 언어의 `ISO639-1` 언어 코드(`zh-CN`, `ja`, `ko` 등)입니다. 전체 언어 코드 및 언어명은 부록 [언어 코드](language-codes.md)를 참조하세요.

### 2단계: 템플릿 파일 내용 업데이트

새로 생성한 `lang/xx.json` 파일, 예를 들어 `lang/ko.json`을 편집하여 모든 영어 내용을 대상 언어로 번역합니다. 다음은 완전한 `한국어` 언어 파일 예제입니다:
...
2. **필드가 완전함**: 필요한 `20`개의 필드가 모두 포함되어 있는지 확인
