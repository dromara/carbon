...
head:
  - - meta
    - name: description
      content: Carbon ライブラリ組み込み定数リファレンスマニュアル、バージョン、タイムゾーン、月、季節、星座、曜日、数値定数および完全なレイアウトテンプレートとフォーマット定数を含み、RFC、ISO8601 などの国際標準フォーマットをカバー
  - - meta
    - name: keywords
      content: バージョン定数, タイムゾーン定数, 月定数, 季節定数, 星座定数, 曜日定数, 数値定数, レイアウトテンプレート定数, RFCフォーマット, ISO8601フォーマット, 時間フォーマット
---
