...
### ステップ 1: 言語テンプレートファイルのコピー
 ...
 ```
ここで `xx` は言語コード（`zh`、`en`、`ja` など）、`YY` は国または地域コード（`CN`、`US`、`CA` など）です。言語コードと国または地域コードが同じ場合は、国または地域コードを省略できます（例：`ru-RU` は `ru` と省略可能）。完全な言語コードと国または地域コードについては、[言語コード](language-codes.md) を参照してください。
